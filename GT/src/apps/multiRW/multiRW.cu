...
struct MultiRWApp {
  ...
  template<typename SampleType>
  __device__ inline
  VertexID next(const int step, VertexID* transit, VertexID sampleIdx, SampleType* sample, VertexID_t numEdges, EdgePos_t neighbrID, curandState* state, GlobalDecoder *gd) {
    if (numEdges == 0) {
      return -1;
    }
    if (numEdges == 1) {
      VertexID_t v = gd->get_vertex(0);
      if (step > 0) {
        sample->rootVertices[sample->lastRootIdx] = v;
      }
      return v;
